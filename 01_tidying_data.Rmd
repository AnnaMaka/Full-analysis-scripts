...
for(i_sub in subjects){
  ...
  for (i_block in blocks){
    ...
    sub_dat <- as_tibble(read_delim(fn, delim = "\t", col_names = FALSE, col_types = "dcdddddddddddddd")) %>%
      ...
      mutate(ID=i_sub, 
             ...
             angleError=NA, landmarkAngle=NA, boundaryAngle=NA, relativeAngle=NA, correct_RA = NA, 
  ...
  for (i_trial in 1:nrow(Sum)) {
    ...
    #calculating the relative influence with a new formula "distCorrect/(distCorrect+distOther)" and creating values in the same direction for both cues
...
Last changes to the summary dataframe
```{r}
#Contemporary chunck for having Age_c datafram loaded in for later graphing
#data loading in wll be removed and the chunck will be joined with the following chunck
...
```{r calculating memory score (random), eval=FALSE}
...
```{r calculating memory score (trajectory), eval=FALSE}
...
```{r organising Sum_all better, eval=FALSE}
Sum_all <- Sum_all %>%
