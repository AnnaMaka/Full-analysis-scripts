...
```{r}
Sum_all <- read_delim(here("data" ,"Sum.txt"), delim = " ", col_names = TRUE, col_types = "fdddddddfcdddddddddddddddddddddddddddddddddddddddd") 
```
...
print(sprintf("The average reaction time for pointing was %s seconds (sd = %s).", round(reaction_time$mean, 3), round(reaction_time$sd, 3)))
```
...
cohen.d(d=summaryBlock1_Angle$angleErr, f=NA, mu=90) 
```
...
t.test(sub_mini_1A$relAngle, mu=0.5, alternative = "greater") %>% tidy()
```
...
t.test(subset(sub_mini_cueA, cue=="landmark")$relAngle, subset(sub_mini_cueA, cue=="boundary")$relAngle, paired=TRUE) %>% tidy()
```
...
#likelihood ratio test
ratioFinalAngle <- anova(modelFullAngle, modelControlAngle)
ratioFinalAngle
```
...
#likelihood ratio test
ratioCueDis_A <- anova(modelCueDis_A, modelControlCueDis_A)
ratioCueDis_A
```
...
#likelihood ratio test
ratioCueDis_cueA <- anova(modelCueDis_cueControlA, modelControlCueDis_A)
ratioCueDis_cueA
```
...
summary(modelCueDis_miniFullA)

#likelihood ratio test
ratioCueDis_fullMiniA <- anova(modelCueDis_miniFullA, modelCueDis_cueControlA)
ratioCueDis_fullMiniA
```
