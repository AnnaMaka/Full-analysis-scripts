...
```{r}
#average RT 
reaction_time_dat <- Sum_angle %>%
  group_by(ID, age) %>%
  summarise(estResponse = mean(secTrialEst), .groups = "drop") 
...
## recoded version analysis
```{r}
formulaFullNext <- "correct_RA ~ age_c*cueMM + ( 1 + cueMM | ID)" 
modelFullNext <- lmer(formula = formulaFullNext, data=subset_RA)
summary(modelFullNext)
```

```{r} 
controlFinalNext <- "correct_RA ~ age_c + cueMM + (1 + cueMM | ID)" 
modelControlNext <- lmer(formula = controlFinalNext, data=subset_RA)
ratioFullModelNext <- anova(modelFullNext, modelControlNext)
ratioFullModelNext
```

```{r}
RA_final <- ggplot(relativeAngleBlocks, aes(age, corCueRA, color=cue)) + 
  theme_cowplot() +
...
```{r }
RA_predict <- ggeffects::ggpredict(modelFullNext, terms = c("age_c", "cueMM")) %>%
  as_tibble() %>%
  mutate(cuePredict = factor(if_else(group == 1, true = "boundary", false = "landmark"),
                      levels = c("boundary", "landmark")))
