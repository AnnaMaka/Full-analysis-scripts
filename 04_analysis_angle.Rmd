...
#likelihood ratio test
ratioAngleError <- anova(modelAngleError, modelControlAngleError) %>% tidy()
```

**$\chi^{2}$(1) = `r round(ratioAngleError$statistic[2],3)`, p`r if (ratioAngleError$p.value[2] > 0.001) {paste("=", round(ratioAngleError$p.value[2], 3))} else {low_p}`**
...
#likelihood ratio test
ratioLandmarkError <- anova(modelLandmarkAngleError, modelControlLandmarkError) %>% tidy()
```

**$\chi^{2}$(1) = `r round(ratioLandmarkError$statistic[2],3)`, p`r if (ratioLandmarkError$p.value[2] > 0.001) {paste("=", round(ratioLandmarkError$p.value[2], 3))} else {low_p}`**
...
#likelihood ratio test
ratioBoundaryError <- anova(modelBoundaryAngleError, modelControlBoundaryError) %>% tidy()
```

**$\chi^{2}$(1) = `r round(ratioBoundaryError$statistic[2],3)`, p`r if (ratioBoundaryError$p.value[2] > 0.001) {paste("=", round(ratioBoundaryError$p.value[2], 3))} else {low_p}`**
...
## Raw Error - Age Mixed Model

**Table**

```{r}
tab_model(modelFullDist, modelAngleError, 
          show.p = FALSE, show.se=TRUE, show.icc=FALSE,
          digits = 4, digits.re = 4,
          pred.labels = c("Intercept","Age", "Cue", "Interaction"),  
          dv.labels = c("Positional Memory", "Pointing"), 
...
```{r}
tab_model(modelAgeLandmark_error, modelAgeBoundary_error, modelLandmarkAngleError, modelBoundaryAngleError, 
          show.p = FALSE, show.se=TRUE, show.icc=FALSE,
          digits = 4, digits.re = 4,
          pred.labels = c("Intercept", "Age"),  
          dv.labels = c("Positional Memory:\nLandmark-dependent", "Positional Memory:\nBoundary-dependent", 
...
g_finalRE <- distance_final + distance_model + AE_final + AE_model +
  plot_layout(design=layout) &
  theme(axis.title = element_text(size = 10), axis.text = element_text(size=10), 
        legend.title = element_text(size=10), legend.text = element_text(size=10), 
        legend.direction = "horizontal", 
        ...
        plot.title = element_text(size=12, face="bold"),
        plot.tag = element_text(size = 10, face="bold", vjust = 3, hjust=-1.5)) &
  plot_annotation(title = 'Raw Error Improvement with Age',
                  theme = theme(plot.title = element_text(size = 12, face="bold")),
                  tag_levels = 'A')

ggsave("rawError.pdf", plot=g_finalRE, 
       units = "cm", width = 15.9, height = 18, dpi = "retina", device = cairo_pdf, path = here("figures"))
ggsave("rawError.png", plot=g_finalRE, 
       units = "cm", width = 15.9, height = 18, dpi = "retina", device = "png", path = here("figures"))
...
```{r}
used_pkgs <-readLines(here("citations.md"))
