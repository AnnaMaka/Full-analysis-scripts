...
This difference in relative influence scores between boundary-dependent objects and landmark-dependent objects is visualiyed in a graph below. 
...
To test, whether cue is a significant predictor we ran a likelihood ration test comparing our simpliest model and a model containing only random effects.
...
As we decided to recode relative influence score to dCorrect / (dCorrect + dOther), we ran the main model with miniblocks again with the new dependent variable to see if the slope differ despite the same direction of improvement.
...
#likelihood ratio test
ratioMini <- anova(modelMiniblocksControl, modelJustCue)
ratioMini
```
